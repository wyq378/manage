...
public class LoginView extends JFrame {
	...
	private void check() {
		...
		CSDAO csDAO = (CSDAO) BaseDAO.getAbilityDAO(DAO.CSDAO);
		CXYDAO cxyDAO = (CXYDAO) BaseDAO.getAbilityDAO(DAO.CXYDAO);
		if (adminDAO.queryForLogin(username.getText(), String.valueOf(password.getPassword()))) {
			dispose();
			new MainView();
		}
		if (csDAO.queryForLogin(username.getText(), String.valueOf(password.getPassword()))) {
			dispose();
			new MainView();
		}
		if (cxyDAO.queryForLogin(username.getText(), String.valueOf(password.getPassword()))) {
			dispose();
			new MainView();
		}
		else {
